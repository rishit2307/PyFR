...
class DtStatsPlugin(BasePlugin):
    ...
    def __init__(self, intg, cfgsect, prefix):
        ...
        if rank == root:
            self.outf = init_csv(self.cfg, cfgsect, 'n,t,dt,action,error,cfl, nsteps, nrej')
        else:
            self.outf = None

    def __call__(self, intg):
        # Process the sequence of rejected/accepted steps
        for i, (dt, act, err, cfl, nsteps, nrej) in enumerate(intg.stepinfo, start=self.count):
            self.stats.append((i, self.tprev, dt, act, err, cfl, nsteps, nrej))

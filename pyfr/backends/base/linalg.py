...
def batched_inv_ok_dtypes(mdtype, odtype):
    ...
    return mdtype == odtype or (mdtype == np.float32 and odtype == np.float16)
...
class BaseGPULinalgKernels:
    ...
    def _inv_tplargs(self, out, mdtype):
        n = out.block_size
        stm, stn, ftm, ftn = self._inv_tiles(mdtype)
        extra = {'sc': self._inv_sc(n, mdtype), 'stm': stm, 'stn': stn,
                 'ftm': ftm, 'ftn': ftn}
    ...
    def _inv_argtypes(self, mdtype):
        ...
        return [np.uintp]*5 + [ixdtype, ixdtype, np.dtype(mdtype).type]
